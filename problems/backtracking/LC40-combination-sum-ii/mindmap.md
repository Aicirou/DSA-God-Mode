# Mind Map: Combination Sum II

```mermaid
graph TD
    A[Combination Sum II] --> B[Key Challenges]
    B --> B1[No Reuse]
    B --> B2[Handle Duplicates]
    
    A --> C[Solution Strategy]
    C --> C1[Sort Array First]
    C --> C2[Backtracking]
    C --> C3[Skip Same-Level Dups]
    
    C2 --> D[Algorithm]
    D --> D1[Base: remaining = 0]
    D --> D2[Loop from start]
    D --> D3[Skip if i > start AND nums i = nums i-1]
    D --> D4[Add to current]
    D --> D5[Recurse with i+1]
    D --> D6[Remove from current]
    
    style A fill:#ff6b6b
    style C fill:#4ecdc4
    style D fill:#ffe66d
```

## Duplicate Handling Visualization

```
candidates = [1,1,2], target = 3

Without skip:        With skip:
    []                  []
   / | \               / \
  1  1  2             1   2
 /|  |              / \
1 2  2            1   2
|                 |
2                 2
Results: [1,2], [1,2]  Results: [1,2]
```
